// utils-concept-map.js — Concept Map (card text color tied only to card background)
// Uses your UI/layout/handlers; updates text color logic and CSS.
// Sources used inline: [1], [2], [3], [4], [5], [6], [7]

console.log('[utils-concept-map.js v77]');
...
const CMAP_MIN_SPLIT_LENGTH = 60;          // from [4], but OK to tune
const ORPHAN_RECOVERY_THRESHOLD = 0.85;    // [4]
const DRAG_DELAY = 600;                    // [1]

// Styles (inject once), based on your [6], with card text tied to --ppc-on/--ppc-bg only
(function injectCmapStyles() {
  if (document.getElementById('pp-cmap-styles')) return;
  const s = document.createElement('style');
  s.id = 'pp-cmap-styles';
  s.textContent = `
#pp-cmap-head { flex-shrink:0; padding:10px 12px 8px; border-bottom:1px solid var(--md-sys-color-outline-variant); background:var(--md-sys-color-surface-container-low); display:flex; flex-direction:column; gap:5px; }
#pp-cmap-subtitle { font-size:11px; font-weight:500; color:var(--md-sys-color-on-surface-variant); letter-spacing:.04em; line-height:1.3; min-height:14px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
#pp-cmap-status { display:inline-flex; align-items:center; gap:6px; padding:4px 8px; border-radius:var(--radius-full); width:fit-content; font-size:9px; font-weight:600; letter-spacing:.07em; text-transform:uppercase; border:1px solid transparent; transition:opacity .6s ease, background .4s ease; }
...
#pp-cmap-layout-btn { height:100%; border:1px solid var(--md-sys-color-outline-variant); border-radius:var(--radius-full); padding:4px 10px; font-size:9px; font-weight:600; letter-spacing:.07em; text-transform:uppercase; background:var(--md-sys-color-surface-container);color:var(--md-sys-color-on-surface-variant); cursor:pointer; transition:background .15s,color .15s,box-shadow .15s; white-space:nowrap; display:flex; align-items:center; gap:4px; }
...
#pp-cmap-layout-menu { position:absolute; top:calc(100% + 5px); right:0; z-index:300; background:var(--md-sys-color-surface-container-lowest);border:1px solid var(--md-sys-color-outline-variant); border-radius:var(--radius-md); box-shadow:var(--md-elev-3); padding:5px; min-width:148px; display:none; flex-direction:column; gap:1px; animation:pp-cmap-menu-in .15s cubic-bezier(0.22,1,0.36,1) both; }
...
.pp-cmap-layout-opt { display:flex; align-items:center; gap:6px; width:100%; border:none; background:transparent; text-align:left; padding:5px 9px; border-radius:var(--radius-sm); cursor:pointer; font-size:9px; font-weight:600; letter-spacing:.03em; color:var(--md-sys-color-on-surface-variant); transition:background .12s; }
...
#pp-cmap-empty { position:absolute; inset:0; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:8px; font-size:11px; letter-spacing:.08em; text-transform:uppercase; color:var(--md-sys-color-on-surface-variant); text-align:center; padding:24px; pointer-events:none; }
#pp-cmap-zoom-hint { position:absolute; bottom:7px; right:9px; font-size:9px; font-weight:600; letter-spacing:.05em; color:var(--md-sys-color-outline); pointer-events:none; z-index:20; }
...
/* Card base (from [6]) — now inherit per-card text color */
.pp-cmap-card { position:absolute; border-radius:9px; border:none; background:var(--ppc-bg,#444); color:var(--ppc-on,#fff); box-shadow:var(--md-elev-2); cursor:grab; user-select:none; overflow:hidden; transition:box-shadow .15s; }
.pp-cmap-card:active { cursor:grabbing; }
.pp-cmap-card:hover { box-shadow:var(--md-elev-4); }

/* Card top layout (from [6]) */
.pp-cmap-card-top { display:flex; align-items:flex-start; justify-content:space-between; padding:5px 9px 4px; gap:6px; }
.pp-cmap-card-cat-num { font-size:11px; font-weight:800; letter-spacing:.02em; line-height:1.35; flex:1; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.pp-cmap-card-level-block { display:flex; flex-direction:column; align-items:flex-end; flex-shrink:0; }
.pp-cmap-card-level-num { font-size:14px; font-weight:900; line-height:1; }
.pp-cmap-card-level-label { font-size:7px; font-weight:700; letter-spacing:.08em; text-transform:uppercase; }
.pp-cmap-card-merged { font-size:8px; font-weight:600; letter-spacing:.06em; text-transform:uppercase; padding:0 9px 3px; }
.pp-cmap-card-rule { height:1px; margin:0 9px; opacity:.5; }

/* All text elements driven only by per-card vars --ppc-on (text) and --ppc-bg (bg) */
.pp-cmap-card .pp-cmap-card-cat-num {
  color: var(--ppc-on,#fff);
  color: color-mix(in srgb, var(--ppc-on,#fff) 92%, var(--ppc-bg,#444));
}
.pp-cmap-card .pp-cmap-card-level-num {
  color: var(--ppc-on,#fff);
  color: color-mix(in srgb, var(--ppc-on,#fff) 95%, var(--ppc-bg,#444));
}
.pp-cmap-card .pp-cmap-card-level-label {
  color: var(--ppc-on,#fff);
  color: color-mix(in srgb, var(--ppc-on,#fff) 55%, var(--ppc-bg,#444));
}
.pp-cmap-card .pp-cmap-card-merged {
  color: var(--ppc-on,#fff);
  color: color-mix(in srgb, var(--ppc-on,#fff) 50%, var(--ppc-bg,#444));
}
.pp-cmap-card .pp-cmap-card-rule {
  background: color-mix(in srgb, var(--ppc-on,#fff) 22%, var(--ppc-bg,#444));
  opacity: 1;
}
.pp-cmap-card .pp-cmap-cell-cat {
  color: var(--ppc-on,#fff);
  color: color-mix(in srgb, var(--ppc-on,#fff) 60%, var(--ppc-bg,#444));
}
.pp-cmap-card .pp-cmap-cell-text {
  color: var(--ppc-on,#fff);
  color: color-mix(in srgb, var(--ppc-on,#fff) 92%, var(--ppc-bg,#444));
}
.pp-cmap-card .pp-cmap-merge-sep {
  background: color-mix(in srgb, var(--ppc-on,#fff) 20%, var(--ppc-bg,#444));
  border-color: transparent;
}
.pp-cmap-card .pp-cmap-card-footer {
  border-top-color: color-mix(in srgb, var(--ppc-on,#fff) 18%, var(--ppc-bg,#444));
}
.pp-cmap-card .pp-cmap-sim-line {
  color: var(--ppc-on,#fff);
  color: color-mix(in srgb, var(--ppc-on,#fff) 60%, var(--ppc-bg,#444));
}
.pp-cmap-card .pp-cmap-sim-pct {
  color: var(--ppc-on,#fff);
  color: color-mix(in srgb, var(--ppc-on,#fff) 92%, var(--ppc-bg,#444));
}
.pp-cmap-card .pp-cmap-leaf-badge {
  color: var(--ppc-on,#fff);
  color: color-mix(in srgb, var(--ppc-on,#fff) 55%, var(--ppc-bg,#444));
}

/* Body (from [6]) — but remove theme text colors, rely on per-card vars */
...
.pp-cmap-split-fraction { font-size:9px; font-weight:900; letter-spacing:.04em; opacity:.8; flex-shrink:0; align-self:center; padding:1px 6px; border-radius:8px; }
.pp-cmap-parent-count {
  font-size:8px; font-weight:700; letter-spacing:.06em; padding:1px 5px; border-radius:8px; flex-shrink:0; align-self:center;
  background: color-mix(in srgb, var(--ppc-on,#fff) 20%, var(--ppc-bg,#444));
  color: color-mix(in srgb, var(--ppc-on,#fff) 90%, var(--ppc-bg,#444));
}
.pp-cmap-merged-count { font-size:8px; font-weight:700; letter-spacing:.07em; padding:1px 6px; border-radius:10px; margin-left:auto; opacity:.7; }

.pp-cmap-card-body { padding:6px 9px 4px; display:flex; flex-direction:column; gap:4px; }
.pp-cmap-merge-sep { border-top:1px solid color-mix(in srgb, var(--ppc-on,#fff) 18%, var(--ppc-bg,#444)); margin:3px 0; }
.pp-cmap-card-footer { padding:4px 9px 7px; border-top:1px solid color-mix(in srgb, var(--ppc-on,#fff) 18%, var(--ppc-bg,#444)); margin-top:2px; display:flex; flex-direction:column; gap:3px; }

.pp-cmap-sim-row { display:flex; align-items:center; gap:5px; }
.pp-cmap-sim-arrow { font-size:9px; font-weight:800; flex-shrink:0; width:10px; text-align:center; opacity:.55; }
...
function initConceptMapTool(paneEl, sidebarEl) {
  ...
  // Upgrade sliders if helper present (from [1])
  ...
  const MM_PAD = 16;  // [1]
  let _depth = 5;                              // [1]
  let _threshold = CMAP_PARENT_CHILD_THRESHOLD;// [1]
  let _maxParents = 1;                         // [1]
  let _layout = 'organic';                     // [1]
  let _rows = null;
  let _rendered = false;
  ...
  // Pointer pan/pinch and card drag (from [5])
  ...
  canvas.addEventListener('pointermove', ev => {
    ...
    if (_pinchState && _pointers.size >= 2) {
      ev.preventDefault();
      const pts = [..._pointers.values()];
      const newMidX = (pts[1].x + pts[2].x) / 2;
      const newMidY = (pts[1].y + pts[2].y) / 2;
      const newDist = Math.hypot(pts[1].x - pts[2].x, pts[1].y - pts[2].y);
  ...
  function makeDraggable(el, cid) {
    ...
    el.addEventListener('pointerup', ev => {
      if (_cardDrag && ev.pointerId === _cardDrag.pointerId) _cardDrag = null;
    });
  ...
  async function maybeSplitRow(row) {
    ...
    return groups.map((segs,ni)=>({
      ...
      kws:row.kws||new Set(), _splitFrom:catStr, _splitN:ni+1, _splitT:t,
  ...
  function buildHierarchy(rows) {
    ...
    for (let i=0;i<n;i++) {
      ...
      if (bestJ!==-1) {
        ...
        parentsOf.set(i,[bestJ]);
        parentSimsOf.set(i,[bestSim]);
      } else levels[i]=1;
    }
    ...
    for (let i=0;i<n;i++) {
      ...
      let sum=0;
      childrenOf[i].forEach(c=>{sum+=cosineSim(rows[i].vec,rows[c].vec);});
      simToChildren[i]=sum/childrenOf[i].length;
    }
    const simToParents=new Float32Array(n);
    for (let i=0;i<n;i++) {
      const pars=parentsOf.get(i)||[];
      if (!pars.length) continue;
      let sum=0;
      pars.forEach(p=>{sum+=cosineSim(rows[i].vec,rows[p].vec);});
  ...
  function redrawConnectors() {
    ...
    _connEdges.forEach(({fromId,toId,color,depth})=>{
      ...
      function pts(r){
        const{x,y,w,h}=r;
        return [{x:x+w*.25,y},{x:x+w*.5,y},{x:x+w*.75,y},{x:x+w*.25,y:y+h},{x:x+w*.5,y:y+h},{x:x+w*.75,y:y+h},{x,y:y+h*.33},{x,y:y+h*.67},{x:x+w,y:y+h*.33},{x:x+w,y:y+h*.67}];
      }
  ...
  // Contrast helper: returns '#fff' or '#000' to maximize contrast against a hex bg
  function contrastFor(hex) {
    ...
    const crW = 1.05 / (L + 0.05);
    const crB = (L + 0.05) / 0.05;
    return crW >= crB ? '#fff' : '#000';
  }

  // Color by level — read from THEMES with fallbacks (shape from [4]/[7])
  function depthColor(level) {
    ...
    return {
      ...
      label: theme['--tab-active-color'] || '#ffffff', // not used for text anymore, but kept for completeness
      bg: theme['--bg-data'] || '#f7f7f8',
    };
  }

  // Render cards + connectors, then layout
  function renderConceptMap(hier) {
    ...
    for (let i=0;i<n;i++) {
      ...
      const { accent /*, label: lc, bg*/ } = depthColor(level); // from [4]/[7]
      ...
      // IMPORTANT: tie text color only to the card background (not the site theme)
    ...
    // Build connectors after cards exist (from [3])
    for (let i=0;i<n;i++) {
      if (absorbedInto[i]!==-1) continue;
    ...
    requestAnimationFrame(()=>{
          ...
          _liveRects.forEach((ra,ka)=>{
            _liveRects.forEach((rb,kb)=>{
              ...
              if(ra.x<rb.x+rb.w+MM_PAD&&ra.x+ra.w+MM_PAD>rb.x&&ra.y<rb.y+rb.h+MM_PAD&&ra.y+ra.h+MM_PAD>rb.y){
                const dR=rb.x+rb.w+MM_PAD-ra.x, dL=ra.x+ra.w+MM_PAD-rb.x, dD=rb.y+rb.h+MM_PAD-ra.y, dU=ra.y+ra.h+MM_PAD-rb.y;
      ...
      function layoutTree(vertical) {
        ...
        function place(id,left,depth){
          ...
          if(vertical) pos.set(id,{x:cx-CARD_W/2,y:rowY}); else pos.set(id,{x:rowY,y:cx-h/2});
          let childLeft=left;
      ...
      function layoutOrganic() { // [2], [3]
        ...
        for (let it=0;it<80;it++) {
          ...
          nodeIds.forEach(id=>{
            const d=Math.sqrt(dx[id]*dx[id]+dy[id]*dy[id])||.01,disp=Math.min(d,temp);
        ...
        nodeIds.forEach(id=>{
          const h=cH(id);
          pos.set(id,{x:px[id]-CARD_W/2,y:py[id]-h/2});
        });
        applyPositions(pos);
      }
  ...
  // Re-render on theme change — layout/colors may move; text color stays per-card now
