...
// Clean rewrite based on working v10 architecture. Changes:
//   • Wheel + pinch zoom on canvas (world gets translate+scale)
//   • ONLY cluster/sub-cluster header bars are draggable (cards are NOT)
...
/* depth-0 body is pannable; we wrap content in an inner div */
.pp-cl-nest[data-depth="0"] > .pp-cl-nest-body { cursor:grab; }
...
.pp-cl-resize-handle::after {
  ...
  width:5px;height:5px;border-right:2px solid rgba(0,0,0,.25);border-bottom:2px solid rgba(0,0,0,.25);border-radius:1px;
}
...
function initClustersTool(paneEl, sidebarEl) {
  ...
  let _pinchD=null;
  canvas.addEventListener('touchstart',ev=>{
    if(ev.touches.length===2){const dx=ev.touches[0].clientX-ev.touches[1].clientX,dy=ev.touches[0].clientY-ev.touches[1].clientY;_pinchD=Math.hypot(dx,dy);}
  },{passive:true});
  canvas.addEventListener('touchmove',ev=>{
    ...
    const d=Math.hypot(dx,dy);_zoom=Math.max(0.15,Math.min(4,_zoom*d/_pinchD));_pinchD=d;applyWorldTransform();ev.preventDefault();
  },{passive:false});
  canvas.addEventListener('touchend',()=>{_pinchD=null;});

  // ── Nest drag (depth-0 and depth-1 heads only) ──────────────────────────────
  let _nestDrag=null;
  function makeNestDraggable(nestEl){
    ...
    head.addEventListener('mousedown',ev=>{
      ...
      nestEl.style.zIndex=String(++_topZ);nestEl.classList.add('pp-cl-nest-lifted');
      hideTooltip();
    });
  ...
  function showTooltip(ev,r,text,accentColor){
    _ttRow=r;
    ttCluster.textContent='';ttCluster.style.color=accentColor;
  ...
  function cosineSim(a,b){let d=0,na=0,nb=0;for(let i=0;i<a.length;i++){d+=a[i]*b[i];na+=a[i]*a[i];nb+=b[i]*b[i];}return na&&nb?d/(Math.sqrt(na)*Math.sqrt(nb)):0;}
  function autoCluster(rows,minK,maxK){
    ...
    while(centers.length<k){
      ...
      const sum=dists.reduce((a,b)=>a+b,0);let r=Math.random()*sum,pick=0;
      for(let i=0;i<n;i++){r-=dists[i];if(r<=0){pick=i;break;}}
      centers.push(vecs[pick]);
    }
    let asgn=new Array(n).fill(0);
    for(let iter=0;iter<20;iter++){
      const na=vecs.map(v=>{let best=0,bs=-Infinity;centers.forEach((c,ci)=>{const s=cosineSim(v,c);if(s>bs){bs=s;best=ci;}});return best;});
      centers.forEach((_,ci)=>{const ms=vecs.filter((_,i)=>na[i]===ci);if(!ms.length)return;const dim=ms[0].length;const sum=new Float32Array(dim);ms.forEach(v=>v.forEach((x,i)=>{sum[i]+=x;}));centers[ci]=sum.map(x=>x/ms.length);});
  ...
  // ── buildCard — content text only, no badge or tab name ───────────────────
  ...
  function buildNestRecursive(rows,depth,maxDepth,path){
    ...
    let children=null;
    if(!isLeaf){
      const minK=depth===0?_outerMin:_innerMin, maxK=depth===0?_outerMax:_innerMax;
